...
interface BookingCartPaneProps {
  cart: any[];
  cartTotal: number;
...
export function BookingCartPane({ className, cart, cartTotal, removeFromCart, updateQuantity, updatePrice, onCheckout }: BookingCartPaneProps) {
    ...
    <div className={`w-full lg:w-1/3 flex flex-col bg-white dark:bg-[#111118] border-l border-slate-200 dark:border-white/5 ${className || ''}`}>
      ...
      <div className="px-4 py-3 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h2 className="text-sm font-bold text-slate-900 dark:text-white tracking-tight">Booking Items</h2>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
        {cart.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-4 py-16 text-center">
            ...
            <div>
              <p className="text-sm font-bold text-slate-400 dark:text-slate-500">No products selected</p>
              <p className="text-xs text-slate-400 dark:text-slate-600 mt-1">Search and select products to pre-book</p>
            </div>
          </div>
        ) : (
          cart.map(item => (
