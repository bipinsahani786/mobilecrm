...
<body>
    ...
    <header>
        ...
        <div class="customer-box">
            <table class="customer-table">
                <tr>
                    <td class="lbl">To</td>
                    <td class="val">: {{ strtoupper($booking->customer->name ?? 'Walk-in Customer') }}</td>
                    <td class="lbl">Booking Receipt No</td>
                    <td class="val">: {{ $booking->booking_number }}</td>
                </tr>
                <tr>
                    <td class="lbl">Phone No</td>
                    <td class="val">: {{ $booking->customer->phone ?? 'N/A' }}</td>
                    <td class="lbl">Date</td>
                    <td class="val">: {{ \Carbon\Carbon::parse($booking->date)->format('d/m/Y') }}</td>
                </tr>
                <tr>
                    ...
                    <td class="val">: 
                        @php
                            $customerAddress = 'N/A';
                            if ($booking->customer) {
                                if (!empty($booking->customer->village) || !empty($booking->customer->city) || !empty($booking->customer->district) || !empty($booking->customer->state)) {
                                    $parts = array_filter([$booking->customer->village, $booking->customer->city, $booking->customer->district, $booking->customer->state]);
                                    $customerAddress = implode(', ', $parts);
                                    if (!empty($booking->customer->pin)) {
                                        $customerAddress .= ' - ' . $booking->customer->pin;
                                    }
                                } elseif (!empty($booking->customer->address)) {
                    ...
                    <td class="val">: {{ $booking->valid_until ? \Carbon\Carbon::parse($booking->valid_until)->format('d/m/Y') : 'N/A' }}</td>
                </tr>
            </table>
        </div>
    </header>
    ...
    <div class="bill-title-container">
        <h2 class="bill-title">BOOKING RECEIPT</h2>
    </div>
    ...
    <div class="summary-wrapper clearfix">
        <div style="float:left; width:50%;">
            <div class="status-badge">BOOKING RECEIPT</div>
        </div>
        ...
        <table class="totals-table">
            ...
            @if($booking->advance_amount > 0)
                <tr>
                    <td style="font-weight:600; color:#16a34a;">Advance Paid</td>
                    <td style="text-align:right; color:#16a34a;">- Rs.{{ number_format($booking->advance_amount, 2) }}</td>
                </tr>
            @endif
            <tr class="grand-total">
                <td>PENDING</td>
                <td style="text-align:right;">Rs.{{ number_format(max(0, $booking->total_amount - $booking->advance_amount), 2) }}</td>
            </tr>
        </table>
    </div>
            ...
            <tr>
                <td style="width: 50%; vertical-align: top; padding-right: 15px;">
                    ...
                    <div style="margin-top: 5px; line-height: 1.4;">
                        @if(!empty($business->settings['booking_terms']))
                            {!! nl2br(e($business->settings['booking_terms'])) !!}
                        @else
